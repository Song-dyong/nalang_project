...
from livekit.agents.cli.cli import run_app  # 실행 진입점 (dev/start)
...
# ---------- 환경변수 ----------
LIVEKIT_URL = os.getenv("LIVEKIT_URL")  # run_app에서 ENV로 읽음
...
# 첫 참가자 들어온 뒤에만 녹음 시작할지 여부 (기본: True)
...
def get_whisper_model():
    ...
    if _WHISPER_MODEL is None:
        _WHISPER_MODEL = whisper.load_model("base")  # 필요 시 small/medium/large
    return _WHISPER_MODEL


# ---------- 유틸 ----------
async def wait_s3_object_exists(bucket: str, key: str, timeout_sec: int = 300) -> bool:
    """egress가 S3에 파일 올릴 때까지 폴링."""
...
async def entrypoint(ctx: JobContext):
    ...
    # 0) 먼저 방에 연결 (경고 제거 + 안정화)
    await ctx.connect()
    if WAIT_FOR_PARTICIPANT:
        # 첫 원격 참가자 올 때까지 대기(빈 방 녹음 방지)
    ...
    # 1) 오디오만 Egress → S3
    # *주의*: egress 워커 컨테이너를 반드시 구동해야 함 (livekit/egress)
    ...
    # 2) 종료 콜백: S3 확인 → 다운로드 → Whisper 전사 → 업로드 → API
    async def on_shutdown(reason: str = ""):
        ...
        # (e) transcript 업로드
        transcript_url = (
            f"https://{S3_BUCKET}.s3.{S3_REGION}.amazonaws.com/{transcript_s3_key}"
        )
        try:
    ...
    # 여기까지 오면 agent는 방에 붙어 이벤트 루프를 유지함
    # 실제 종료는 모든 참가자 퇴장 → on_shutdown 호출로 이어짐
